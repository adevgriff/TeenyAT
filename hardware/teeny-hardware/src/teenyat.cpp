...
bool tny_init_from_file(teenyat *t,
								TNY_READ_FROM_BUS_FNPTR bus_read,
								TNY_WRITE_TO_BUS_FNPTR bus_write) // TODO: file io like this is not going to work probably want to find a way to store program on something seperate and external with lots of reads and writes
{
	if (!t)
		return false;
	t->initialized = false;
	if (picoTest_bin_len == 0)
		return false;
	...
	/* backup .bin file */
	uint16_t temp = 0;
	for (unsigned int i = 0; i < picoTest_bin_len / 2; i++)
	{
		temp = picoTest_bin[(i * 2) + 1];
		temp <<= 8;
		temp |= picoTest_bin[i * 2];
		t->bin_image[i].u = temp;
	}
...
void tny_clock(teenyat *t)
{
	...
	default:
		...
		Serial.print(opcode);
		Serial.print(") encountered at: ");
		Serial.print(orig_PC, t->cycle_cnt);
		Serial.print("\n");
		break;
	}
