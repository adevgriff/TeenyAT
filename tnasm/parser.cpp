...
shared_ptr<token> p_include_line();
bool p_raw_line();
...
shared_ptr<token> p_include_line()
{
    shared_ptr<token> val = nullptr, path;
    shared_ptr<tny_word> immed;
    if (term(T_INCLUDE) && (path = term(T_STRING)) && term(T_EOL))
    {
        if (pass == 1)
        {
            string file_path = path->token_str;
...
bool p_raw_line()
{
    ...
    while (all_good)
    {
        ...
        else if ((tnext = save, s = term(T_STRING)))
        {
            string characters = s->token_str;
            characters = characters.substr(1, characters.length() - 2);
            for (unsigned int i = 0; i < characters.length(); i++)
            {
                data.push_back(shared_ptr<tny_word>(new tny_word{.u = static_cast<tny_uword>(characters[i])}));
            }
        }
